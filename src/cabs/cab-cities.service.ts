...
@Injectable()
export class CabCitiesService implements OnModuleInit {
  constructor(
    @InjectRepository(CabCity)
    private cabCityRepository: Repository<CabCity>,
  ) {}
  ...
  async seed() {
    ...
    // Optional: Clear existing cities if you only want New York
  ...
  async findBySlug(slug: string): Promise<CabCity | null> {
